...
export default function Edit({card}: Props) {
        ...
        const handleColorSet = (color: string) => {
            ...
            colorsArr.map((color, key) => {
                let val = {label: color, value: color}
                colors.push(val)
        })
            return colors
        }
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Edit" />
                ...
                <div className='flex flex-col gap-2'>
                    ...
                    <Select value={data.game} onValueChange={(value)=>setData('game', value)}>
                ...
                <div className="flex flex-col gap-2">
                    <Label>Cor</Label>
                        <MultipleSelector  className="w-[280px]" value={handleColorSet(data.color)} onChange={(values) => handleMultiSelect(values)} defaultOptions={optionsDcg} placeholder='Selecione uma cor'/>
                </div>}
                {data.game === "Magic" && 
                <div className="flex flex-col gap-2">
                    <Label>Cor</Label>
                        <MultipleSelector  className="w-[280px]" value={handleColorSet(data.color)} onChange={(values) => handleMultiSelect(values)} defaultOptions={optionsMtg} placeholder='Selecione uma cor'/>
                </div>}
